...
public:
  void init(PinConfig config) {
    size_t port_idx = static_cast<size_t>(std::distance(
        gpio_ports.begin(),
...
private:
  IRQn_Type _irqn = NonMaskableInt_IRQn; // use NonMaskableInt_IRQn as value for
                                         // not initialized interrupt
  static inline std::array<void (*)(void), 16> on_interrupt = {
      emb::invalid_function, emb::invalid_function, emb::invalid_function,
      emb::invalid_function, emb::invalid_function, emb::invalid_function,
      emb::invalid_function, emb::invalid_function, emb::invalid_function,
      emb::invalid_function, emb::invalid_function, emb::invalid_function,
      emb::invalid_function, emb::invalid_function, emb::invalid_function,
      emb::invalid_function,
  };
...
public:
  ...
  virtual void
  set(emb::gpio::pin_state s = emb::gpio::pin_state::active) override {
    assert(_initialized);
...
public:
  ...
  AlternatePin(const PinConfig &config) {
    assert(config.pin.Mode == GPIO_MODE_AF_PP ||
           config.pin.Mode == GPIO_MODE_AF_OD);
    init(config);
  }
};
...
public:
  ...
  DurationLogger(DurationLoggerChannel channel, DurationLoggerMode mode)
    ...
    if (_mode == DurationLoggerMode::set_reset) {
      ...
      mcu::write_reg(_pin->port->BSRR,
                     ((odr_reg & _pin->pin) << 16u) | (~odr_reg & _pin->pin));
